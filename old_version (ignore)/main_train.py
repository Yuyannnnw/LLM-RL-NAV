...
def call_llm_for_shaping(prev_obs, next_obs, action):
    ...
    Observation space is a matrix where each row represents a vehicle (first row is myself) and columns are [relative x, relative y, heading (radians), velocity x, velocity y].
...
class EnvWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        if self.mode == 'Hybrid':
            shape_reward = call_llm_for_shaping(self.prev_obs, next_obs, action) / 10.0
...
def main():
    ...
    monitor_dir = build_logs_path(logs_base, env_id, mode, observation_type, run_id)
    ...
    # 8) Build path for model: models/env_id/mode/run_id/model
    folder_name = "models"
